...
class Reflection : public Material, public Light{
protected:
...
public:    
	void init(GLuint heightmapTex, vector<GLuint> tabMixingTex){
        // Compile the shaders
        _pid = opengp::load_shaders("_reflection/reflection_vshader.glsl", "_reflection/reflection_fshader.glsl");
        ...
        // Vertex coordinates and indices
        {
            ...
            int reflection_dim = 512;

			//Generate the mesh            
			for (int i = 0; i <= reflection_dim; i++){
				for (int j = 0; j <= reflection_dim; j++){
					vertices.push_back(-1.0f + i * 2 / (float)reflection_dim); vertices.push_back(-1.0f + j * 2 / (float)reflection_dim);
				}
			}

			int indice = 0;
			for (int row = 0; row<terrain_dim; row++){
				for (int col = 0; col<reflection_dim; col++){
					indices.push_back(indice);
					indices.push_back(indice + 1);
					indices.push_back(indice + reflection_dim + 2);
					indices.push_back(indice);
					indices.push_back(indice + reflection_dim + 1);
					indices.push_back(indice + reflection_dim + 2);
					indice++;
				}
				indice++;
		...
		glfwLoadTexture2D("_reflection/texture-herbe.tga", 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		...
		glfwLoadTexture2D("_reflection/snow.tga", 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		...
		glfwLoadTexture2D("_reflection/rock2.tga", 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		...
		glfwLoadTexture2D("_reflection/montagneNeige2.tga", 0);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
